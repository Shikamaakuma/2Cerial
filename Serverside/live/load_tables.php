<?php
    ...
    $QUERYWINTERTHUR  = "select Temperature, AirPressure, AirQuality, WaterSaturation, unix_timestamp(subdate(Datum, interval second(Datum) second))*1000 from Readings WHERE Readings.Datum > DATE_SUB(now(), INTERVAL 1 HOUR) AND UserID = 2;";
    $QUERYNEUHAUSEN = "select Temperature, AirPressure, AirQuality, WaterSaturation, unix_timestamp(subdate(Datum, interval second(Datum) second))*1000 from Readings WHERE Readings.Datum > DATE_SUB(now(), INTERVAL 1 HOUR) AND UserID = 3;";
